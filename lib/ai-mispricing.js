...
function materializeCase(candidate, side) {
  ...
  return {
    ...
    finalVerdict: candidate.finalVerdict || null,
  };
}
...
function splitAiMispricingCandidates(candidates, limit = 12) {
  ...
  for (const candidate of candidates || []) {
    const finalClass = candidate.finalVerdict?.classification;
    if (finalClass === "undervalued") {
      undervalued.push(materializeCase(candidate, "undervalued"));
      continue;
    }
    if (finalClass === "overvalued") {
      overvalued.push(materializeCase(candidate, "overvalued"));
      continue;
    }
    if (finalClass === "insufficient") {
      rejected.push({
        symbol: candidate.symbol,
        companyName: candidate.companyName,
        reason: candidate.finalVerdict?.summary || "证据不足",
        lowScore: caseScore(candidate, "undervalued"),
        highScore: caseScore(candidate, "overvalued"),
      });
      continue;
    }
